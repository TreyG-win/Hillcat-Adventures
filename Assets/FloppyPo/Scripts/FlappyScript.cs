...
public class FlappyScript : MonoBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (GameStateManager.GameState == GameState.Playing)
        {
            if (col.gameObject.tag == "Pipeblank") //pipeblank is an empty gameobject with a collider between the two pipes
            {
                ...
                ScoreTracker.FloppyPoScore = ScoreManagerScript.Score;
            }
    ...
    void FlappyDies()
    {
        ...
        if(!ObjectiveManager.floppyPoQuestComplete)
        {
            ObjectiveManager.floppyPoQuestComplete = true;
        ...
        //Once the player is dead, then the sanity is drained.
        Managers.Player.FloppyPoSanityChanges(ScoreTracker.FloppyPoScore);
